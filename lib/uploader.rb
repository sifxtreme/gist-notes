...
module GistNotes
  class Uploader
    ...
    def folder_paths_to_sync
      ...
      folders.select { |x| folder_syncable?(x) }
    end

    def folder_syncable?(folder)
    ...
    def sync_root_index_file_up
      ...
      gists_on_device_and_online = gists.select do |x|
        gists_on_device.include?(x[:description])
      end
    ...
    def delete_folder(path)
      refresh_gists
      folder_name, = path.gsub(Config::GIST_NOTES_PATH, '').split('/')
    ...
    def get_folder_name(path)
      path.gsub(Config::GIST_NOTES_PATH, '').split('/').reject(&:empty?).first
    end
    ...
    def md_for_gist_files(gist)
      gist[:files].map do |_, file|
        # we created a special index file in a gist
        next if file[:filename] == "_#{gist[:description]}.md"
